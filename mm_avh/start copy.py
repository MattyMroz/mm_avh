...
from mm_avh.modules.mkv_processing import SubtitleProcessing
...
def process_output_files(settings: Settings):
    ...
    files_dict = {file.split('.')[0]: [] for file in files}
    for file in files:
        files_dict[file.split('.')[0]].append(file)

    for base_name, files in files_dict.items():
        if len(files) > 0:
            subtitle_processor = SubtitleProcessing(base_name)
